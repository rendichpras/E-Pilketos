...
export default function AdminTokensPage() {
  ...
  const tokens = useMemo(() => page.tokensRes?.tokens ?? [], [page.tokensRes]);
  ...
  useEffect(() => {
    const electionId = page.selectedElectionId;
    if (!electionId) return;

    void loadTokens(electionId, pageNumber, pageSize);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page.selectedElectionId, pageNumber, pageSize, tab, query]);

  useEffect(() => {
    if (pageNumber !== 1) setPageNumber(1);
  }, [tab, query, pageNumber]);
  ...
  async function handleGenerate(e: FormEvent) {
    ...
    try {
      await adminApi.tokens.generate(page.selectedElectionId, {
        count,
        batch: form.batchLabel.trim() || undefined
      });
  ...
  async function doInvalidate(token: Token) {
    ...
    try {
      await adminApi.tokens.invalidate(token.id);
      await loadTokens(page.selectedElectionId, pageNumber, pageSize);
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal mengubah status token."
          : "Gagal mengubah status token.";
      setPage((prev) => ({ ...prev, error: message }));
      toast.error(message);
    } finally {
