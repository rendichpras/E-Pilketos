...
import type { Election, ElectionStatus, UpdateElectionDto } from "@/lib/types";
...
export default function AdminElectionsPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (editingId) {
        const payload: UpdateElectionDto = {
          name,
          description
        };
        ...
        const updated = await adminApi.elections.update(editingId, payload);

        setElections((prev) => prev.map((e) => (e.id === updated.id ? updated : e)));
        toast.success("Pemilihan berhasil diperbarui.");
      } else {
        const created = await adminApi.elections.create({
          slug,
          name,
          description,
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal menyimpan pemilihan. Periksa kembali data Anda."
          : "Gagal menyimpan pemilihan. Periksa kembali data Anda.";
      setError(message);
      toast.error(message);
      setSubmitting(false);
    }
  ...
  async function doActivate(election: Election) {
    ...
    try {
      const updated = await adminApi.elections.activate(election.id);
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal mengaktifkan pemilihan sebagai aktif."
          : "Gagal mengaktifkan pemilihan sebagai aktif.";
      setError(message);
      toast.error(message);
    } finally {
  ...
  async function doClose(election: Election) {
    ...
    try {
      const updated = await adminApi.elections.close(election.id);
      setElections((prev) => prev.map((e) => (e.id === updated.id ? updated : e)));
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal menutup pemilihan."
  ...
  async function doPublishResults(election: Election) {
    ...
    try {
      const updated = await adminApi.elections.publishResults(election.id);
      setElections((prev) => prev.map((e) => (e.id === updated.id ? updated : e)));
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal mempublikasikan hasil."
          : "Gagal mempublikasikan hasil.";
      setError(message);
      toast.error(message);
    } finally {
  ...
  async function doHideResults(election: Election) {
    ...
    try {
      const updated = await adminApi.elections.hideResults(election.id);
      setElections((prev) => prev.map((e) => (e.id === updated.id ? updated : e)));
      ...
      const message =
        err instanceof ApiError
          ? err.message || "Gagal menyembunyikan hasil."
          : "Gagal menyembunyikan hasil.";
      setError(message);
      toast.error(message);
    } finally {
      setPublishBusyId(null);
